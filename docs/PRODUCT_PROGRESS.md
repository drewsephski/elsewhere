...
- Bot chat reuses one primary Postgres conversation per bot. Follow-up sends share message history for the Responses engine (excluding the in-flight user/assistant pair) and resume the same Codex app-server thread via `thread/resume`, with `thread/compact/start` at 24/48/72 completed-turn milestones.
