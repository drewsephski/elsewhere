...
pub async fn build_run_input_messages(
    ...
    let mut messages =
        load_bounded_responses_history(pool, conversation_id, bot_id, assistant_message_id).await?;
...
mod tests {
    ...
    fn compaction_runs_on_interval_milestones_only() {
        assert_eq!(codex_compact_milestone_due(23, 0, 24), None);
        assert_eq!(codex_compact_milestone_due(24, 0, 24), Some(24));
        assert_eq!(codex_compact_milestone_due(47, 24, 24), None);
    ...
    async fn responses_history_excludes_inflight_user_and_assistant(pool: PgPool) {
        ...
        let history = load_bounded_responses_history(&pool, &conv_id, &bot_id, &asst_current)
            .await
            .unwrap();
