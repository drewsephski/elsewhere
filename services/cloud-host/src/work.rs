...
pub async fn enqueue_from_group_message_in_transaction(
    ...
    let run_id = Uuid::new_v4().to_string();
    sqlx::query(
        "INSERT INTO agent_runs (id, owner_id, request_id, bot_id, conversation_id, computer_id, model, status, assistant_message_id, source_message_id) VALUES ($1,$2,$3,$4,$5,$6,$7,'queued',$8,$9)",
    )
    .bind(&run_id)
    .bind(owner)
    .bind(request_id)
