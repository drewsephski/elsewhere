...
export function WorkspaceOverview({ compact = false }: { compact?: boolean }) {
  ...
  return <div className="space-y-5">
    {error ? <p role="alert" className="text-sm text-warning">{error}. Displayed activity may be out of date.</p> : null}
    {data && !data.runnerReady ? <p role="status" className="rounded-xl border border-warning/25 bg-warning/10 p-3 text-sm text-warning-foreground">Background work is temporarily unavailable. Queued assignments stay saved while the runner reconnects.</p> : null}
    ...
    <section className="surface-card">
      ...
      {!data ? <p className="mt-4 text-sm text-muted-foreground">Loading your team…</p> : !data.bots.length ? <div className="py-8"><Bot className="h-8 w-8 text-foreground" /><h3 className="mt-4 text-base font-medium">Give your first bot a job</h3><p className="mt-2 max-w-lg text-sm leading-6 text-muted-foreground">Connect ChatGPT from the overview, create a computer, and give your bot a name and role. Then delegate a report, a draft, or a task from your project.</p><div className="mt-4 flex gap-4 text-sm"><Link href="/app/computers" className="font-medium underline underline-offset-4">Create a computer</Link><Link href="/app/bots" className="font-medium underline underline-offset-4">Create a bot</Link></div></div> : <ul className="mt-4 grid gap-3 xl:grid-cols-2">{data.bots.map(bot => {
