...
export function ResultsPanel({ runId }: { runId?: string }) {
  ...
  async function download(item: ResultItem) {
    setDownloading(item.id); setError(null);
    try {
      const response = await cloudHostFetch(`/v1/results/${item.id}/download`);
      if (!response.ok) throw new Error("Could not download this result");
  ...
  return <section className="surface-card">
    ...
    {!items.length ? <p className="mt-4 text-sm text-muted-foreground">{loading ? "Loading results…" : collecting ? "Results will be saved here when your bot finishes." : "No saved results yet. Delegate work to a bot and ask for a report, draft, or file."}</p> : <ul className="mt-4 divide-y divide-border">{items.map(item => <li key={item.id} className="flex flex-wrap items-center justify-between gap-3 py-4 first:pt-0 last:pb-0">
      ...
      <button type="button" className="flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm disabled:opacity-50" onClick={() => void download(item)} disabled={downloading !== null}><Download className="h-4 w-4" />{downloading === item.id ? "Downloading…" : "Download"}<span className="sr-only"> {item.name}</span></button>
    </li>)}</ul>}
